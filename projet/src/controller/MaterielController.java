...
public class MaterielController implements Initializable {
    ...
    @FXML
    private ComboBox<String> combox_location;
    @FXML
    ...
    private void ajouter(ActionEvent event) {
        if (tf_nom_materiel.getText().isEmpty() || tf_description_materiel.getText().isEmpty()
            ...
            m.setLocation(combox_location.getValue());
    ...
    private void handleAction(MouseEvent event) throws SQLException {
        ...
        combox_location.setValue(m.getLocation());
        ...
        if (m == null) {
           ...
           if (tf_nom_materiel.getText().isEmpty() || tf_description_materiel.getText().isEmpty()
            ...
            JOptionPane.showMessageDialog(null, "Veuillez saisir un prix valable");
                ...
                mc.update(m.getId(), tf_nom_materiel.getText(),tf_description_materiel.getText(),Float.valueOf(tf_prix_materiel.getText()),tf_photo_materiel.getText(),combox_location.getValue());
                initTable();
                clearAll();
            }
        }
    ...
    @FXML
    private void comboChange_location(ActionEvent event) {
    }

    @FXML
    private void Detail_Location_materiel(ActionEvent event) throws SQLException{
        if (!lmc.affiche(Integer.valueOf(label_id_materiel.getText())))
        {
            try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/view/location_materiel.fxml"));
            Parent root = loader.load();
            Location_materielController dp = loader.getController();
            dp.setMaterielId(label_id_materiel.getText());
            dp.setlabel_location_materiel_ajouter(tf_nom_materiel.getText());
          
            tf_nom_materiel.getScene().setRoot(root);
        } catch (IOException ex) {
            Logger.getLogger(Location_materielController.class.getName()).log(Level.SEVERE, null, ex);
        }
        }
        else{
            try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/view/location_materiel_modif.fxml"));
            Parent root = loader.load();
            Location_materiel_modifController dp = loader.getController();
            dp.setMaterielId(label_id_materiel.getText());
            dp.settf_location_materiel_total(String.valueOf(lmc.returnID(Integer.valueOf(label_id_materiel.getText()))));
            dp.setlabel_location_materiel_ajouter(tf_nom_materiel.getText());
            
            tf_nom_materiel.getScene().setRoot(root);
        } catch (IOException ex) {
            Logger.getLogger(Location_materiel_modifController.class.getName()).log(Level.SEVERE, null, ex);
        }
        }
    }
